...
/// Try to determine if the complex number `c` is in Mndelbrot set, using at most `limit`
...
fn parse_complex(s: &str)-> Option<Complex<f64>> {
    match parse_pair(s, ',') {
        Some((re, im)) => Some(Complex {re, im}), // Used shorthand notation
        None => None
    }
}
...
/// The `upper_left` and `lower_right` parameters are the points on the complex
...
fn main() {
    ...
    // Instead of render, we can use the below code to run the calculations in parallel
    let threads = 8;
